...
class ConfigV2:
    ...
    CYCLE_EPOCHS = 20      # 每个余弦退火周期的 Epoch 数量 (T_0)
...
class CosineAnnealingWarmRestartsDecay(CosineAnnealingWarmRestarts):
    def __init__(self, optimizer, T_0, T_mult=1, eta_min=0, decay=1.0):
    ...
    def step(self, epoch=None):
        ...
        if self.T_cur == 0:
            for i, group in enumerate(self.optimizer.param_groups):
                group['lr'] = self.base_max_lrs[i] * self.decay
                self.base_max_lrs[i] = group['lr']
...
def train():
    ...
    #     eta_min=1e-6
    # )
    main_scheduler = CosineAnnealingWarmRestartsDecay(
    optimizer,
    T_0=cfg.CYCLE_EPOCHS,
    T_mult=2,
    eta_min=1e-6,
    decay=0.8   # 每次重启后最大学习率衰减到 80%
    )
    ...
    if rank == 0:
        if best_model_path_synced:
            print(f"\n[GPU {device_id}] Evaluating BEST model: {os.path.basename(best_model_path_synced)}")
    ...
    if world_size > 1 and rank == 1:
        if second_best_model_path_synced:
            print(f"\n[GPU {device_id}] Evaluating 2ND BEST model: {os.path.basename(second_best_model_path_synced)}")
            my_results = evaluate_model(cfg, second_best_model_path_synced, scaler_save_path, device=f"cuda:{device_id}")
            print(f"[GPU {device_id}] Finished evaluating 2ND BEST model.")
        else:
...
def evaluate_model(train_cfg, model_path, scaler_path, device):
    ...
    with torch.no_grad(), amp.autocast(device_type='cuda', dtype=torch.float16, enabled=True):
        for tensors in tqdm(test_dataloader, desc="Evaluating"):
