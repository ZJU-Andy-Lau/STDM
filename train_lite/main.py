...
def train():
    ...
    if rank == 0:
        ...
        print(f"Optimization: Dense Graph Ops + Max Channels {cfg.MAX_CHANNELS}")
    ...
    model = SpatioTemporalDiffusionModelV2(
        ...
        max_channels=cfg.MAX_CHANNELS # 传入参数
    ).to(device_id)
            ...
            for micro_idx in range(current_batch_size):
                ...
                with context:
                    with amp.autocast():
                        ...
                        # [修改] 传入 adj_tensor 而不是 micro_edge_data
            ...
            for tensors in val_dataloader:
                ...
                with amp.autocast():
                    ...
                    pred_noise = ddp_model(
                        x_k, k, history_c_p, static_c, future_known_c_p, adj_tensor
                    )
