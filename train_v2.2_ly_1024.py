...
class ConfigV2:
    ...
    # 训练参数
    EPOCHS = 100
    BATCH_SIZE = 16 # 注意：这是【单张卡】的batch size
    LEARNING_RATE = 1e-4
...
def periodic_evaluate_mae(model, loader, scaler, edge_index, edge_weights, cfg, device):
    ...
    for (history_c, static_c, future_x0_true, future_known_c) in progress_bar:
        ...
        # --- 核心修改: 累加局部的(local)误差和计数 ---
        abs_error = np.abs(denorm_pred - denorm_true).sum()
        count = np.prod(denorm_true.shape) # 元素总数
...
def train():
    ...
    original_edge_index = original_edge_index.to(device_id)
    original_edge_weights = original_edge_weights.to(device_id)
    ...
    # 1. 在所有 rank 上创建验证集子集 (Subset)
    val_eval_indices = list(range(len(val_dataset)))
    # 我们只取配置中指定的前 N 个 batch 来加速评估
    ...
    # 2. 为这个子集创建分布式采样器 (DistributedSampler)
    val_eval_sampler = DistributedSampler(
        val_eval_dataset,
    ...
    # 3. 在所有 rank 上创建 MAE 评估所需的数据加载器
    val_eval_loader = DataLoader(
        val_eval_dataset,
        batch_size=cfg.BATCH_SIZE,
        sampler=val_eval_sampler, # 使用 DDP 采样器
        num_workers=4,
        pin_memory=True
    )
    ...
    optimizer = optim.AdamW(ddp_model.parameters(), lr=cfg.LEARNING_RATE * world_size, weight_decay=1e-4)
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if rank == 0:
            ...
            #         second_best_model_path_for_eval = model_save_path_second_best

                best_val_loss = avg_val_loss
                print(f"🎉 New valid loss min model with validation loss: {best_val_loss:.4f}")
            #     torch.save(ddp_model.module.state_dict(), model_save_path_best)
            #     best_model_path_for_eval = model_save_path_best
            #     print(f"🎉 New best model saved to {model_save_path_best} with validation loss: {best_val_loss:.4f}")
                ...
                print(f"🥈 New valid loss 2nd min model with validation loss: {second_best_val_loss:.4f}")
        ...
        if run_mae_eval:
            if rank == 0:
                print(f"Epoch {epoch+1}, running DISTRIBUTED periodic MAE evaluation...")
            
            # --- 核心修改: 所有 rank 都参与评估 ---
            # 传入 ddp_model.module (原始模型)
            local_abs_error, local_count = periodic_evaluate_mae(
                ddp_model.module, 
                val_eval_loader,
                train_scaler,
                original_edge_index,
            ...
            # --- 核心修改: 聚合(reduce)所有 rank 的结果 ---
            # 1. 将 Python 浮点数/整数 转换为 GPU 上的 Tensor
            local_error_tensor = torch.tensor(local_abs_error, dtype=torch.float64, device=device_id)
            local_count_tensor = torch.tensor(local_count, dtype=torch.float64, device=device_id)
            
            # 2. 使用 all_reduce 将所有 rank 上的 Tensor 相加
            dist.all_reduce(local_error_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(local_count_tensor, op=dist.ReduceOp.SUM)
            
            # 3. 从 Tensor 中获取全局(global)的总和
            global_abs_error = local_error_tensor.item()
            global_count = local_count_tensor.item()
            
            # 4. 计算全局 MAE (所有 rank 上的值都一样)
            if global_count > 0:
                current_val_mae = global_abs_error / global_count
            else:
                current_val_mae = float('inf') # 避免除以零
            ...
            if rank == 0:
                ...
                if run_mae_eval and current_val_mae < best_val_mae:
                    ...
                    torch.save(ddp_model.module.state_dict(), model_save_path_best)
                    best_model_path_for_eval = model_save_path_best
                    print(f"🎉 New best model saved to {model_save_path_best} with validation MAE: {best_val_mae:.4f}")

                elif run_mae_eval and current_val_mae < second_best_val_mae:
                    second_best_val_mae = current_val_mae
                    torch.save(ddp_model.module.state_dict(), model_save_path_second_best)
                    second_best_model_path_for_eval = model_save_path_second_best
                    print(f"🥈 New 2nd best model saved to {model_save_path_second_best} with validation MAE: {second_best_val_mae:.4f}")
    ...
    path_list_to_eval = [best_model_path_for_eval, second_best_model_path_for_eval] if rank == 0 else [None, None]
    dist.broadcast_object_list(path_list_to_eval, src=0)

    best_model_path_synced, second_best_model_path_synced = path_list_to_eval
    ...
    if rank == 0:
        ...
        if metrics_second_best:
            ...
            print("\n===== [FINAL RESULT 2/2] 2ND BEST Model: SKIPPED (not saved or error) =====")
