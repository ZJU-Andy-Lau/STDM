...
class ConfigV2:
    ...
    # [核心修改] 损失函数权重配置
    # 1. 均值约束 (Mean MSE): 强迫 K 个样本的中心逼近真值 (解决均值偏移)
    MEAN_MSE_LAMBDA = 10.0       
    
    # 2. 个体约束 (Individual L1): 强迫每个样本都靠近真值 (解决过度离散/倒U型)
    INDIVIDUAL_L1_LAMBDA = 1.0   
    
    # 3. 排斥约束 (Repulsion): 鼓励样本之间保持距离 (防止坍缩)
    # Loss = ... - REPULSION_LAMBDA * distance_between_samples
