...
class EVChargerDatasetV2(Dataset):
    def __init__(self, features, history_len, pred_len, cfg, scaler=None):
        ...
        target_col_log = np.log1p(target_col_original)

        if scaler is None:
            self.scaler = self._initialize_scaler(target_col_log)
        else:
            self.scaler = scaler

        if self.cfg.NORMALIZATION_TYPE != "none":
            target_col_reshaped = target_col_log.reshape(-1, 1)
            normalized_target = self.scaler.transform(target_col_reshaped)
            dynamic_features[:, :, 0] = normalized_target.reshape(target_col_log.shape)
...
def periodic_evaluate_mae(model, loader, scaler, edge_index, edge_weights, cfg, device):
    ...
    for (history_c, static_c, future_x0_true, future_known_c, idx) in progress_bar:
        ...
        pred_norm = median_prediction.cpu().numpy()
        true_norm = future_x0_true.permute(0, 2, 1, 3).cpu().numpy()

        if scaler is not None:
            # 先从 [-1,1] 或 z-score 空间 inverse_transform 回到 log(y+1) 空间
            pred_log = scaler.inverse_transform(pred_norm.reshape(-1, 1)).reshape(pred_norm.shape)
            true_log = scaler.inverse_transform(true_norm.reshape(-1, 1)).reshape(true_norm.shape)

            # 再从 log 空间反变换回原始空间：y = exp(log_y) - 1
            denorm_pred = np.expm1(pred_log)
            denorm_true = np.expm1(true_log)
        else:
            # 如果完全没用 scaler（NORMALIZATION_TYPE = "none"），这里就直接当作原始值用
            denorm_pred = pred_norm
            denorm_true = true_norm
...
def evaluate_model(train_cfg, model_path, scaler_path, device, rank, world_size,key):
    ...
    if rank == 0:
        ...
        # 1. 在 rank 0 上进行反归一化
        if scaler:
            all_predictions_log = scaler.inverse_transform(all_predictions_norm.reshape(-1,1)).reshape(all_predictions_norm.shape)
            y_true_log = scaler.inverse_transform(all_true_norm.reshape(-1,1)).reshape(all_true_norm.shape)
            all_samples_log = scaler.inverse_transform(all_samples_norm.reshape(-1,1)).reshape(all_samples_norm.shape)

            # === 2. inverse transform from log-space → original space ===
            all_predictions = np.expm1(all_predictions_log)
            y_true_original = np.expm1(y_true_log)
            all_samples = np.expm1(all_samples_log)
        else:
