...
@torch.no_grad()
def periodic_evaluate_mae(model, loader, scaler_y, adj_tensor, cfg, device):
    """
    在验证集子集上运行 MAE 评估。
    此函数只应在 rank 0 上调用。
    """
    ...
    all_true_list = []
    for (history_c, static_c, future_x0, future_known_c, idx, start_idx) in progress_bar:
        tensors = [d.to(device) for d in (history_c, static_c, future_x0, future_known_c)]
        ...
        for _ in range(cfg.EVAL_ON_VAL_SAMPLES):
            sample = model.ddim_sample(
                ...
                shape=future_x0.permute(0, 2, 1, 3).shape, 
        ...
        median_prediction = torch.median(stacked, dim=0).values          # (B,N,L,1)

        y_pred = median_prediction.detach().cpu().numpy()
        y_true = future_x0.permute(0, 2, 1, 3).detach().cpu().numpy()
...
def evaluate_model(train_cfg, model_path, scaler_y_path, scaler_mm_path, scaler_z_path, device, rank, world_size,key):
    ...
    with torch.no_grad(), amp.autocast():
        for tensors in tqdm(test_dataloader, desc="Evaluating", disable=disable_tqdm):
            history_c, static_c, future_x0, future_known_c, idx, start_idx = tensors
            ...
            all_true_list.append(future_x0.permute(0, 2, 1, 3).cpu().numpy())
            ...
            for _ in range(cfg.NUM_SAMPLES):
                sample = model.ddim_sample(
                    ...
                    future_known_c=future_known_c.permute(0, 2, 1, 3),
                    adj=adj_tensor, # 传入
                    shape=future_x0.permute(0, 2, 1, 3).shape, sampling_steps=cfg.SAMPLING_STEPS,
                    eta=cfg.SAMPLING_ETA
                )
                generated_samples.append(sample)
    ...
    if rank == 0:
        ...
        if scaler_y:
            y_pred = scaler_y.inverse_transform(all_predictions_norm.reshape(-1, 1)).reshape(all_predictions_norm.shape)
            y_true = scaler_y.inverse_transform(all_true_norm.reshape(-1, 1)).reshape(all_true_norm.shape)
            y_samp = scaler_y.inverse_transform(all_samples_norm.reshape(-1, 1)).reshape(all_samples_norm.shape)
        else:
            y_pred = all_predictions_norm
            y_true = all_true_norm
            y_samp = all_samples_norm
